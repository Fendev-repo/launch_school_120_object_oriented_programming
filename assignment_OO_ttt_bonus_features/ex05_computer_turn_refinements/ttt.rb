...
class TTTGame
  ...
  def reset_first_player
    if @current_marker != @first_to_move
      @current_marker = @first_to_move
    end
  end
  ...
  def choose_who_moves_first
    if @current_marker == 'choose'
      first_plyr = get_human_to_choose_who_moves_first
  ...
  def get_human_to_choose_who_moves_first
    reply = nil
    puts "Would you like to go first? 'y' or 'n'"  
    loop do 
      reply = gets.chomp
      break if ['y', 'n'].include? reply
      puts "You entered #{reply} - I need a 'y' or 'n'"
    end
  ...
  def computer_moves
    if board.opportunity_to_win
      board[board.opportunity_to_win] = computer.marker
    elsif board.square_under_threat
      board[board.square_under_threat] = computer.marker
    elsif board.square_five_available?
