...
class RPSGame
  attr_accessor :human, :computer, :scorecard
  ...
  def congratulate_winner
    puts "#{who_won} was victorius: With #{Scorecard::WINNING_SCORE} wins first"
  end

  def who_won
    hum_mv = human.move
    comp_mv = computer.move

    return 'tie'      if hum_mv.value == comp_mv.value
    return 'human'    if hum_mv > comp_mv
    return 'computer' if hum_mv < comp_mv
  end
